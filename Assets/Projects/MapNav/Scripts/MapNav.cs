...
public class MapNav : MonoBehaviour 
{
	...
	IEnumerator Start () {
		...
		GameController.Instance.SetLoadingBar (0f);
	...
	void MyPosition(){
		if(gpsFix){
			if(!simGPS){
				...
				GameController.Instance.SetAccuracy(Mathf.RoundToInt(loc.horizontalAccuracy));
			}
			else{
	...
	IEnumerator Online(){
		if(!mapDisabled){
			...
			if(www.error == null){
				...
				if(!GameController.Instance.IsGameStarted()) GameController.Instance.GeolocationCompleted();
